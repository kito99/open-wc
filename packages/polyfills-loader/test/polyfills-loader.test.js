...
const { createPolyfillsLoader, getPolyfills } = require('../index');

const updateSnapshots = process.argv.includes('--update-snapshots');

const defaultPolyfills = getPolyfills({
  coreJs: true,
  fetch: true,
  intersectionObserver: true,
  webcomponents: true,
}).map(pf => ({ ...pf, hash: 'mockhash' }));

function testSnapshot({ name, entries, legacyEntries = null, polyfills = null, minified = false }) {
  const snapshotPath = path.join(__dirname, 'snapshots', 'loader-script', `${name}.js`);
  const script = createPolyfillsLoader(entries, legacyEntries, polyfills, minified);
